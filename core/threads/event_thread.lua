...
return {make=function(ENV,BUS,args)
    return coroutine.create(function()
        while true do
            ...
            if ev[1] == keypressed.ev then keypressed.run(BUS,ENV.iso,unpack_ev(ev)) end
            if ev[1] == keyreleased.ev then keyreleased.run(BUS,ENV.iso,unpack_ev(ev)) end
            if ev[1] == textinput.ev then textinput.run(BUS,ENV.iso,unpack_ev(ev)) end

            if ev[1] == mousemoved.ev then mousemoved.run(BUS,ENV.iso,unpack_ev(ev)) end
            if ev[1] == mousepressed.ev then mousepressed.run(BUS,ENV.iso,unpack_ev(ev)) end
            if ev[1] == mousereleased.ev then mousereleased.run(BUS,ENV.iso,unpack_ev(ev)) end
            if ev[1] == wheelmoved.ev then wheelmoved.run(BUS,ENV.iso,unpack_ev(ev)) end

            if ev[1] == "mouse_drag" then mousemoved:check_change(BUS,ENV.iso,ev[3],ev[4]) end
        end
    end)
end}